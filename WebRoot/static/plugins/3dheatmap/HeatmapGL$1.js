...
class HeatmapGL extends WebGLHeatmap {
  ...
  render(gl,view_matrix){
    ...
    var matrix_gray = matrix_set.getProjMatrix.call(this.map.transform,this.heights.width,this.heights.height,4);
    var matrix_display = matrix_set.getProjMatrix.call(this.map.transform,this.heights.width,this.heights.height,1);

    var resolution = 1.4062500000000002 / Math.pow(2,this.map.getZoom())*0.4;
        ...
        if(type == "MultiPoint"){
          for (j = coords.length - 1; j >= 0 ; j --) {
            items.push({
              ...
              intensity:features[i].geometry.height/100
            });
          }
        }else{
          items.push({
...
///////////////////////////////////////////////////
   ...
   */
  setMaxHeight(height){
    this.config.maxHeight = (height || 0) * 1000/1000*60;
