...
WebGLHeatmap = (function() {
  function WebGLHeatmap(_arg) {
    ...
    var gridx = this.gridx = 1600; //Math.floor(gridy / (this.height / this.width));
   ...
   * @param    {array}   extent 范围
   */
  WebGLHeatmap.prototype.display = function(matrix, extent) {
...
class HeatmapGL extends WebGLHeatmap {
  constructor(options) {
    super({
      gl: options.gl,
      intensityToAlpha: true,
  ...
  render(gl,matrix){
    ...
    this.extent = extent;
    if(this.visible){
      this.update(matrix,resolution,extent);
      this.display(matrix,extent);
    }
  }
  /**
   * 设置热力数据
  ...
  setSource(data){
    ...
    //         radius = radius * properties[field + "_rat"];
    //       }
    //     }
    //     if(type == "MultiPoint"){
   ...
   */
  setMaxHeight(height){
    this.config.maxHeight = (height || 0) * 1000/1000*600;
    this.map.triggerRepaint();
  }
  /**
   * 设置热力渐变色
  ...
  setRadius(radius){
    ...
    while(true){
      ...
      if(features[i / 8] && field){
        properties = features[i / 8].properties;
        if(field in properties){
          radius = radius * properties[field + "_rat"];
        }
      }
